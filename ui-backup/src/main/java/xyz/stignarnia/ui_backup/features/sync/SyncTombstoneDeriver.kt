...
/**
 * Works out what this device deleted, by comparing local state against the state it last published.
 *
 * The alternative was recording every deletion as it happens, at every call site that removes something.
 * That is around sixty places once the hidden ones are counted - moving a show into the collection quietly removes it from the watchlist and from hidden - and each addition would have to clear its tombstone again.
 * One missed site is a deletion that never propagates, which surfaces months later as a show that keeps coming back.
 *
