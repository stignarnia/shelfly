...
/**
 * Accumulates this device's deletions and keeps them alive long enough for
 * every peer to hear about them.
 *
 * A deletion derived from the diff is only visible for the cycle that found it.
 * If it were published once and dropped, a device that happened to be offline
 * during that single sync would never see it, and would re-add the item on its
 * next merge - the deletion silently undone by a peer that simply was not
 * listening at the right moment.
 *
 * So derived deletions are persisted and re-published every cycle until they
 * expire. The window is the price of that guarantee: a device offline for
 * longer than [SettingsSyncRepository.TOMBSTONE_RETENTION_DAYS] may still
 * resurrect what it never learned was deleted.
 */
internal class SyncTombstoneStore @Inject constructor(
  ...
  /**
   * Brings the store up to date for this cycle and returns everything still
   * worth publishing.
   *
  ...
  suspend fun refresh(
    ...
    // Anything present again was re-added here since it was deleted. This
    // device is authoritative about its own state, so the record goes rather
    // than being left for timestamp ordering to settle.
  ...
  /**
   * Records deletions learned from a peer, so this device re-publishes them
   * too. Without this a deletion would only ever be carried by the device that
   * made it, and would vanish from the network the moment that device stopped
   * syncing or its window expired.
   */
