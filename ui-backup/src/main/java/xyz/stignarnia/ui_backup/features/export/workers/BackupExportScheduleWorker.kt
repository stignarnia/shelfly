...
class BackupExportScheduleWorker @AssistedInject constructor(
  ...
  companion object {
    ...
    /** Percentage complete, published while the work runs. See [BackupSyncStage]. */
  ...
  override suspend fun doWork(): Result {
    ...
    try {
      ...
      // Reported either way: a failed prune still reports success, so the
      // stage is behind us regardless of which branch got here.
   ...
   *
   * WorkManager keeps this only while the work is RUNNING and drops it when the
   * run ends, so an observer needs no "finished" stage - the progress going
   * away IS the run being over, whether it succeeded or failed.
   */
