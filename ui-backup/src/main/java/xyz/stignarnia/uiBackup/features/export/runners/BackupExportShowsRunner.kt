...
internal class BackupExportShowsRunner
  @Inject
  constructor(
    ...
    private suspend fun exportShowsRatings(): BackupShows =
      withContext(dispatchers.IO) {
        val showsRatings =
          ratingsRepository.loadShowsRatings().map {
            BackupShowRating(
              tmdbId = it.idTmdb.id,
              rating = it.rating,
    ...
    private suspend fun exportSeasonsRatings(): BackupShows =
      withContext(dispatchers.IO) {
        val seasonsRatings =
          ratingsRepository.loadSeasonsRatings().map { rating ->
            BackupSeasonRating(
    ...
    private suspend fun exportEpisodesRatings(): BackupShows =
      withContext(dispatchers.IO) {
        val episodesRatings =
          ratingsRepository.loadEpisodesRatings().map { rating ->
            BackupEpisodeRating(
              showTmdbId = rating.idTmdb,
              seasonNumber = rating.seasonNumber,
