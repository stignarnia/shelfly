...
import xyz.stignarnia.uiBackup.features.export.targets.BackupDestination
import xyz.stignarnia.uiBackup.features.export.targets.BackupEntry
import xyz.stignarnia.uiBackup.features.export.targets.LocalFolderBackupDestination
import xyz.stignarnia.uiBackup.features.export.targets.WebDavBackupDestination
import xyz.stignarnia.uiBackup.features.sync.BackupSyncStage
import xyz.stignarnia.uiBackup.features.sync.SyncEngine
 ...
 */
@HiltWorker
class BackupExportScheduleWorker
  @AssistedInject
  constructor(
    ...
    companion object {
      ...
      fun schedulePeriodic(
        ...
        val request =
          PeriodicWorkRequestBuilder<BackupExportScheduleWorker>(schedule.duration, schedule.durationUnit)
            .setInputData(data)
      ...
      fun scheduleOneOff(workManager: WorkManager) {
        val request =
          OneTimeWorkRequestBuilder<BackupExportScheduleWorker>()
            .setConstraints(
              Constraints
                .Builder()
                .setRequiredNetworkType(NetworkType.CONNECTED)
    ...
    private fun resolveDestination(): BackupDestination =
      when (webDavRepository.backupTarget) {
        ...
        BackupTarget.LOCAL_FOLDER -> {
          val directoryUri =
            inputData.getString(ARG_DIRECTORY_URI)?.toUri()
