 ...
 *
 * Read local state, work out what this device has deleted since it last published, take in every peer's state, merge, apply the result locally, and publish what this device now holds.
 *
 * Sync is not backup, and this does not write snapshots.
 * The scheduled job still does that separately, on its own retention - otherwise the fork would lose the property it was built for, since sync propagates an accidental deletion rather than protecting against one.
 */
class SyncEngine
  @Inject
  internal constructor(
     ...
     *
     * A failure is written down rather than only thrown, because the screen has to be able to say that syncing is broken.
     * Reporting the last success alone would leave a device that has been failing for a week looking merely idle.
     *
     * [onStage] is called as each stage of the cycle completes, for callers that show how far along a run is.
     * It is reporting only - a caller that does not care passes nothing, and a cycle behaves identically either way.
     */
    suspend fun sync(
    ...
    private suspend fun runCycle(
      ...
      // What this device last published is the only baseline it has for spotting its own deletions: anything in there and not here now is gone.
      val published = remoteSource.downloadOwn(credentials, deviceId)
      val tombstones =
        tombstoneStore.refresh(
          published = published?.state,
          local = local,
          // Dated at the previous sync, not now.
          // All that is known is that the deletion happened somewhere in that window, and the earlier bound lets a genuine re-add elsewhere win the tie.
      ...
      // Re-publish peers' deletions as well as our own.
      // Without this a deletion would only ever be carried by the device that made it, and would leave the network as soon as that device stopped syncing.
      // A tombstone the merge overruled is kept too - it is a claim, not a verdict, and it will keep losing to the newer sighting until the store expires it.
      tombstoneStore.adopt(merged.tombstones)
      onStage(BackupSyncStage.MERGED)

      // Removals first: the importer only ever adds, so running it first would re-add what the merge just decided is gone.
      ...
      // Only once the upload has landed.
      // A failed publish must not move the baseline, or the deletions it was carrying would never be re-derived.
