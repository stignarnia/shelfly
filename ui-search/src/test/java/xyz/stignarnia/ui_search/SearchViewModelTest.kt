...
import xyz.stignarnia.ui_search.cases.SearchFiltersCase
import xyz.stignarnia.ui_search.cases.SearchInvalidateItemCase
import xyz.stignarnia.ui_search.cases.SearchQueryCase
import xyz.stignarnia.ui_search.cases.SearchRecentsCase
import xyz.stignarnia.ui_search.cases.SearchSortingCase
import xyz.stignarnia.ui_search.cases.SearchSuggestionsCase
import xyz.stignarnia.ui_search.cases.SearchTranslationsCase
import xyz.stignarnia.ui_search.helpers.TestData
...
class SearchViewModelTest : BaseMockTest() {
  ...
  @Test
  fun `Should preload suggestions`() =
    runTest {
  ...
  @Test
  fun `Should load recent searches`() =
    runTest {
  ...
  @Test
  fun `Should clear recent searches`() =
    runTest {
  ...
  @Test
  fun `Should not run search if query is blank`() =
    runTest {
      SUT.search("  ")
  ...
  @Test
  fun `Should not store recent search if query is blank`() =
    runTest {
  ...
  @Test
  fun `Should store recent search if query is not blank`() =
    runTest {
  ...
  @Test
  fun `Should not update filters if they are the same`() =
    runTest {
  ...
  @Test
  fun `Should not update sort order if they are the same`() =
    runTest {
      val item = mockk<SearchListItem>()
      coEvery { searchQueryCase.searchByQuery(any()) } returns listOf(item)
      coEvery { searchFiltersCase.filter(any(), any()) } returns true
  ...
  @Test
  fun `Should clear suggestions properly`() =
    runTest {
      val item = mockk<SearchListItem>()
  ...
  @Test
  fun `Should not load suggestions if query length is less than 2`() =
    runTest {
      val item = mockk<SearchListItem>()
      coEvery { suggestionsCase.loadSuggestions(any()) } returns listOf(item)
  ...
  @Test
  fun `Should load suggestions properly`() =
    runTest {
      val item = mockk<SearchListItem>()
  ...
  @Test
  fun `Should update missing image for show properly`() =
    runTest {
      val item = TestData.SEARCH_LIST_ITEM.copy(
        show = Show.EMPTY.copy(title = "test"),
      )
      coEvery { showsImagesProvider.loadRemoteImage(any(), any(), any()) } returns
  ...
  @Test
  fun `Should update missing image for movie properly`() =
    runTest {
      val item = TestData.SEARCH_LIST_ITEM.copy(
        movie = Movie.EMPTY.copy(title = "test"),
      )
      coEvery { moviesImagesProvider.loadRemoteImage(any(), any(), any()) } returns
        Image.createUnavailable(ImageType.POSTER)
      coEvery { searchQueryCase.searchByQuery(any()) } returns listOf(item)
      coEvery { recentSearchesCase.saveRecentSearch(any()) } just Runs
      val job = launch(UnconfinedTestDispatcher()) { SUT.uiState.toList(stateResult) }
  ...
  @Test
  fun `Should update missing suggestion image for show properly`() =
    runTest {
      val item = TestData.SEARCH_LIST_ITEM.copy(
        show = Show.EMPTY.copy(title = "test"),
      )
  ...
  @Test
  fun `Should update missing suggestion image for movie properly`() =
    runTest {
      val item = TestData.SEARCH_LIST_ITEM.copy(
        movie = Movie.EMPTY.copy(title = "test"),
      )
  ...
  @Test
  fun `Should update missing suggestion translation for show properly`() =
    runTest {
      val item = TestData.SEARCH_LIST_ITEM.copy(
        show = Show.EMPTY.copy(title = "test"),
      )
  ...
  @Test
  fun `Should update missing suggestion translation for movie properly`() =
    runTest {
      val item = TestData.SEARCH_LIST_ITEM.copy(
        movie = Movie.EMPTY.copy(title = "test"),
      )
  ...
  @Test
  fun `Should not update missing suggestion translation if default language`() =
    runTest {
  ...
  @Test
  fun `Should not update missing suggestion translation if already has translation`() =
    runTest {
      coEvery { searchTranslationsCase.getLanguage() } returns "pl"
