...
import xyz.stignarnia.uiModel.Person.Job
import xyz.stignarnia.uiPeople.R
import xyz.stignarnia.uiPeople.databinding.ViewPeopleListItemBinding
...
class PeopleListItemView : FrameLayout {
  ...
  fun bind(item: PeopleListItem.PersonItem) {
    ...
    with(binding) {
      ...
      viewPersonItemHeader.text =
        when (mainJob) {
          Job.DIRECTOR -> {
            context.getString(R.string.textDirector)
          }

          Job.WRITER, Job.STORY -> {
            context.getString(R.string.textWriting)
          }

          Job.SCREENPLAY -> {
            context.getString(R.string.textScreenplay)
          }

          Job.MUSIC, Job.ORIGINAL_MUSIC -> {
            context.getString(R.string.textMusic)
          }

          else -> {
            when (item.person.department) {
