package xyz.stignarnia.uiWidgets.search
 ...
 *
 * It stands apart from [xyz.stignarnia.uiWidgets.BaseWidgetProvider]: there is no list, no label bar and nothing to read out of the settings table, so it takes the palette directly rather than inheriting a blocking load it has no use for.
 */
@AndroidEntryPoint
class SearchWidgetProvider : AppWidgetProvider() {
  companion object {
    ...
    fun requestUpdate(context: Context) {
      ...
      val intent =
        Intent(applicationContext, SearchWidgetProvider::class.java).apply {
          val ids: IntArray =
            AppWidgetManager
  ...
  private fun updateWidget(
    ...
    val remoteViews =
      RemoteViews(context.packageName, R.layout.widget_search).apply {
        ...
        setTextViewText(R.id.searchWidgetText, context.getString(R.string.textSearchFor))
