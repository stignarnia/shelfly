...
@Singleton
class CalendarRecentsGrouper
  @Inject
  constructor() : CalendarGrouper {
    override fun groupByTime(
      ...
      val last7DaysItems =
        (items - yesterdayItems).filter {
          val dateDays = it.movie.released
          dateDays?.isAfter(now.minusDays(8)) == true
        }
      val last30DaysItems =
        (items - yesterdayItems - last7DaysItems).filter {
          val dateDays = it.movie.released
          dateDays?.isAfter(now.minusDays(31)) == true
        }
      val last90Days =
        (items - yesterdayItems - last7DaysItems - last30DaysItems).filter {
          val dateDays = it.movie.released
          dateDays?.isAfter(now.minusDays(91)) == true
        }

      val itemsMap =
        mutableMapOf<Int, List<CalendarMovieListItem>>()
          .apply {
