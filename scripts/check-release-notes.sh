...
#
# Verifies two things:
#   - the first line is "Shelfly <versionName>", with versionName read from versions.gradle.
...
versions_file="$root/versions.gradle"
...
# versions.gradle holds the single source of truth, as "versionName: '4.0.6'".
...
if [[ "$actual" != "$expected" ]]; then
  echo "error: release notes heading is out of sync with versions.gradle." >&2
